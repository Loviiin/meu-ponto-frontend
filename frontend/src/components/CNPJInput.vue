...
import { formatCNPJ, onlyDigits, validarCNPJ } from '../utils/validators'
...
const inputId = `cnpj-input-${Math.random().toString(36).substr(2, 9)}`
const error = ref('')
const touched = ref(false)

const isValid = computed(() => {
  if (!props.modelValue) return false
  return validarCNPJ(props.modelValue)
})
...
const handleBlur = () => {
  ...
  if (props.required && !digits) {
    error.value = 'CNPJ é obrigatório'
    return
  }
  
  if (digits && digits.length < 14) {
    error.value = 'CNPJ deve conter 14 dígitos'
    return
  }
  
  if (digits && !validarCNPJ(props.modelValue)) {
    error.value = 'CNPJ inválido. Verifique os números digitados'
    return
  }
...
watch(() => props.modelValue, (newVal) => {
  if (touched.value && newVal) {
    ...
    if (digits.length === 14 && !validarCNPJ(newVal)) {
      error.value = 'CNPJ inválido. Verifique os números digitados'
    }
  }
})
